 ...
 *
 * <p>For array-restructuring ops ({@code add}, {@code remove} at an index), leaf-level
 * mutations are emitted for every affected element so that {@code DirtyPropagator} can
 * match concrete paths (e.g. {@code $.items[1].price}) to wildcard graph nodes (e.g.
 * {@code $.items[*].price}) and propagate to dependent derivations. The parent array
 * is also included so the document is correctly truncated or extended.
 */
class JsonPatchTranslator {
     ...
     */
    static Map<String, JsonNode> translate(JsonNode patchOps, ObjectNode patchedDoc) {
