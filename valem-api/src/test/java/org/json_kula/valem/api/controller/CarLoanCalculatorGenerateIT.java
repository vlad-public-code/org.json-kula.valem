...
/**
 * End-to-end test: calls the real LLM (OpenRouter minimax/minimax-m2.5) to generate a
 * car loan/lease calculator spec, then registers and exercises it via the REST API.
 *
 * Skipped automatically when OPENROUTER_API_KEY is not set.
 */
@SpringBootTest
@AutoConfigureMockMvc
class CarLoanCalculatorGenerateIT {

    private static final Logger log = LoggerFactory.getLogger(CarLoanCalculatorGenerateIT.class);

    private static final String MODEL_ID = "car-loan-calculator";
    private static final String DOMAIN_DESCRIPTION =
            "Generate model for car loan/lease calculator with details on each month and total period.";
    ...
    @Test
    void generate_car_loan_calculator_model_and_register_it() throws Exception {
        Assumptions.assumeTrue(specGenerator != null,
                "Skipping: LLM not configured (set OPENROUTER_API_KEY)");

        // ── Step 1: generate spec via LLM ────────────────────────────────────
        log.info("Generating car loan/lease calculator model spec via LLM...");
        ...
        assertThat(spec.derivations())
                .as("spec must have derivations computing monthly payments and totals")
                .isNotEmpty();
        ...
        String evolutionRequest =
                "Add an 'extraDownPayment' field (number, default 0) representing " +
                "an additional one-time payment made upfront. Subtract it from the " +
                "principal before computing monthly payments and total cost.";
        ...
        // ── Step 7: mutate with the newly introduced extra-down-payment field ─────
        ...
        if (evolvedStatus == 200) {
            log.info("Post-evolution mutation accepted — extra down payment field active");
        } else {
            log.warn("Post-evolution mutation returned {} " +
                    "(LLM may have used a different field name for extraDownPayment)", evolvedStatus);
        }
    ...
    private void collectMutations(ObjectNode mutation, String prefix, JsonNode properties) {
        ...
        while (it.hasNext()) {
            ...
            // Skip array properties — we can't easily build per-element mutations
            ...
            switch (type) {
                // An "integer" schema field rejects a float value (e.g. 60.0) with a 422 type
                // violation, so emit a whole number for integer fields and a double for "number".
                case "integer"           -> mutation.put(path, (long) sampleNumber(name));
                case "number"            -> mutation.put(path, sampleNumber(name));
    ...
    /** Returns a plausible numeric value for loan/lease domain field names. */
    private double sampleNumber(String fieldName) {
        String lower = fieldName.toLowerCase();
        if (lower.contains("amount") || lower.contains("principal") || lower.contains("price"))
            return 25000;
        if (lower.contains("down") || lower.contains("deposit"))
            return 5000;
        if (lower.contains("rate") || lower.contains("interest"))
            return 5.0;
        if (lower.contains("term") || lower.contains("month") || lower.contains("period"))
            return 60;
        if (lower.contains("residual") || lower.contains("balloon"))
            return 5000;
