...
- `library` — optional named JSONata functions/values, bound in **every** expression as `$name(...)` (the function-level counterpart of `constants`). Compiled once per distinct `(define, constants, signatures)` through the process-wide content-addressed `LibraryCache`; merged exports reach every evaluator through the single `EvalBindings.forModel` seam (plus `ViewEvaluator`). **A library cannot read the model document** — its closures are rooted at the definition's own (null) input, so a field name in a function body always evaluates to nothing. That is what keeps a library call free of dependency-graph consequences: every document value arrives as an argument, so the field names stay in the *caller's* expression where `ExpressionPathExtractor` sees them. `LibraryPurityCheck` rejects a document reference as an ERROR because the failure is otherwise a silently blank field. Stored as an ordered `layers` list (a branch prepends its template's layers; later layers win a collision). See [model-spec/schema-and-values.md](docs/reference/model-spec/schema-and-values.md).
...
- `viewDefinition` — optional UI component tree (evaluated by `valem-view`); see [view-system.md](docs/reference/view-system.md) and [model-spec/views.md](docs/reference/model-spec/views.md)
...
- `ExpressionCache` is per-runtime; expressions are compiled once (expensive javac round-trip). Every compiled expression carries `valem.limits.expression-timeout-ms` (default 2000, `0` disables), applied at compile time and resolved per compilation. Derivations/constraints/effect triggers evaluate **inside the model lock**, so an unbounded expression holds the lock rather than failing one request; exceeding the budget degrades to the ordinary evaluation-error path (`null` + a `DerivationTrace`).
