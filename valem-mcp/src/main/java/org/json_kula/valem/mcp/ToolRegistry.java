    ...
    private void register() {
        ...
        if (service instanceof BrowserPairable pairable) {
            add("pair_browser", "Pair with browser",
                ...
                + "{status:\"pending\", verificationUri, verificationUriComplete, userCode, expiresInSec} "
                + "if the developer hasn't approved yet — show them verificationUriComplete when present "
                + "(it already carries the confirmation code, so they only click Approve) and mention "
                + "that the code on that screen should read userCode; fall back to verificationUri, "
                + "which requires them to TYPE userCode. Then "
