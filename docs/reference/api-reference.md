...
| `GET` | `/models/{id}/graph` | Structural dependency-graph projection (nodes `BASE`/`DERIVED`/`META`/`CONSTRAINT`/`EFFECT`, dependency edges, topological levels, per-node expression) powering the "Why is this number?" surface. Read-only; a pure function of the compiled spec. 404 for an unknown model. |
| `GET` | `/models/{id}/verification` | Trust-layer report: runs the spec's embedded self-tests and returns `{state (green/amber/neutral), checkedCount, passedCount, unverifiableCount, cases[]}`. `checkedCount` counts only auto-checkable cases; the report attests to internal consistency for example inputs, **not** to factual correctness. Read-only, pure function of the spec, cached per version. 404 for an unknown model. |
...
| `GET` | `/llm/interactions` | All recorded `LlmInteractionRecord` (prompt + response) pairs, each naming the `provider` and `model` that answered it. Content is redacted when `valem.llm.log.capture-content=false`; `provider`/`model` are metadata and survive that. |
