...
| `valem.llm.repair-temperature` | `0.2` | Sampling temperature on **repair** attempts. Lower = more focused fixes. |
...
| `valem.llm.structured-output.enabled` | `true` | Send the `ModelSpec`/`SpecEvolution` JSON Schema as the provider `response_format` (`json_schema` mode) so the output shape is provider-enforced. OpenAI-compatible providers only; Anthropic ignores it (the schema is also in the prompt). Non-strict (a spec embeds an arbitrary JSON Schema, which strict mode cannot represent). |
