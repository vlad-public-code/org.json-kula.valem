...
| `valem.limits.expression-timeout-ms` | `2000` | Wall-clock budget for **one** expression evaluation; `0` disables. Derivations, constraints and effect triggers evaluate **inside the model lock**, so a non-terminating expression holds the lock rather than merely failing a request — where Loom gives parallelism 1 that can wedge the service. Exceeding it fails that one field (`null` value + an error trace), exactly like any other evaluation error. Real expressions run in single-digit milliseconds, but the deadline is wall-clock and keeps advancing while a thread is descheduled, so a pathologically loaded host can time out a normally-fast expression — raise it, or set `0`, if that happens. Resolved per compilation, so a change takes effect for expressions compiled afterwards without a restart. Also reads `VALEM_LIMITS_EXPRESSION_TIMEOUT_MS`. |
...
| `valem.llm.structured-output` | `schema` | How much `response_format` constraint to ask an OpenAI-compatible provider for: `schema` (json_schema when a response schema is supplied, else json_object), `json` (never json_schema), or `none` (omit `response_format`). Support is a ladder, not a yes/no — most hosted providers accept json_object, fewer accept json_schema, and some local servers (llama.cpp, older vLLM/LM Studio) reject the field outright, answering **400**. Turn it down if a provider that should work returns 400. Safe to do: the generator recovers JSON from prose, so the cost is a higher retry rate rather than a broken pipeline. Ignored by Anthropic, which has no `response_format`. |
...
| `valem.llm.eval-tool.max-calls` | `25` | Max `eval_jsonata` evaluations per attempt (replenished each generation attempt so a repair can re-test its fixes; local, no network). |
| `valem.llm.domain-guidance.enabled` | `true` | Offer the `get_domain_guidance` tool: the model reads the domain description in **any language**, picks matching topic ids from a fixed menu, and gets vetted "hard shape" instructions back (amortization schedules, progressive tax/fee charges, classification, date math, percentages, unit conversion, compound growth, weighted averages, eligibility, proration, …). Replaces keyword matching; the resolved guidance is re-injected into repair prompts. |
| `valem.llm.domain-guidance.topics-file` | *(unset)* | Path to an operator JSON file that **extends or overrides** the builtin topics (`valem-core` resource `valem/domain-guidance-topics.json`). Array of `{ "id", "description", "instructions" }` (or `"instructionLines": [ … ]`); merged **by id** — a matching id replaces the builtin topic, a new id is added. A missing/malformed file is ignored (builtin topics still load). The MCP server honours the same property, or the `VALEM_DOMAIN_GUIDANCE_TOPICS_FILE` env var. |
