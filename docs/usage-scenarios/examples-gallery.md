...
| `net-salary-estonia-2026.json` | A gross-to-net pay calculator under Estonia's 2026 rules (22% income tax over a €700 basic exemption, unemployment + funded-pension deducted first); a clean single-input derivation chain with a `keyValueList` breakdown. |
| `car-tax-estonia.json` | Estimates Estonia's annual motor-vehicle tax and one-time registration tax from a car's CO₂ and mass — threshold arithmetic (`max(0, …)`) feeding `statTile`s and a breakdown. |
| `tic-tac-toe.json` | A playable game against a computer opponent: the current player, the winner across all eight lines, and the draw state are **derived** from nine cell fields, and a `timer` effect plays O's move 2 seconds after yours (win/block/center/corner/edge, computed entirely in the `response.set` JSONata). |
