...
| `tic-tac-toe.json` | A playable game as a pure reactive model: the current player, the winner across all eight lines, and the draw state are **derived** from nine cell fields — the `button` grid mutates cells, and the status updates live with no game code. |
